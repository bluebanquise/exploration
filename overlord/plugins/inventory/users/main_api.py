...
def call_plugin(action: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    Instantiate UserPlugin and execute a specific action with payload.
    """
    ...
    config_path = f"{plugins_path}/inventory/user/config.yml"
    try:
...
api.add_resource(UserListResource, "/api/v1/inventory/user")
api.add_resource(UserResource, "/api/v1/inventory/user/<string:username>")
